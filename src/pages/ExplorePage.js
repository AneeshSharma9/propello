...
function ExplorePage() {
    ...
    const fetchDataFromFirestore = async () => {
        ...
        const ratingData = await fetchData('ratings');
    ...
    const ratingFor = (card) => {
        const providerRatings = ratings.filter((r) => r.providerId === card.userId);
        if (!providerRatings.length) return null;
        const avg = providerRatings.reduce((sum, r) => sum + Number(r.rating), 0) / providerRatings.length;
        return { avg, count: providerRatings.length };
    }

    const renderStars = (avg) => {
        const rounded = Math.round(avg);
        return (
            <span className="stars" aria-label={`${avg.toFixed(1)} out of 5 stars`}>
                {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className={star <= rounded ? "star filled" : "star"}>★</span>
                ))}
            </span>
        );
    };
    ...
    return (
        <>
            ...
            <div className="container" style={{ paddingTop: '1.5rem', paddingBottom: '3rem' }}>
                <h1 className="customh1 mb-2">Browse Services</h1>
                <p style={{ color: 'var(--text-light)', marginBottom: '1.5rem', fontSize: '0.95rem' }}>
                    Find a student developer for your next project.
                </p>
                                        ...
                                        <div className="card-body" style={{ padding: '1.25rem' }}>
                                            ...
                                            {rating && (
                                                <p className="mb-2">
                                                    ...
                                                    <span className="ml-1" style={{ fontSize: '0.8rem', color: 'var(--text-light)' }}>
